...
internal sealed class MyGameManager : GameManager
{
    ...
    {
        // Provide a custom serializer implementation
    }

    protected override IStorage CreateRuntimeStorage()
    {
...
var resources = GameManager.GetResources<MyResource>();

// Retreive a single resource
if (GameManager.TryGetResource<MyResource>("some-path", out var resource))
{
}

// Read resource from StreamingAssets asynchronously
var resource = await GameManager.ReadResourceAsync<MyResource>("some-path")

// Read a stream from StreamingAssets driectory asynchronously
var resource = await GameManager.ReadResourceStreamAsync("some-path")

// Read data
if (GameManager.TryReadData<MyData>("some-key", out var data))
{
}

// Read data asynchronously
var data = await GameManager.ReadDataAsync<MyData>("some-key");

// Save data
GameManager.SaveData("some-key", data);

// Save data asynchronously
await GameManager.SaveDataAsync("some-key", data);

// Delete data
GameManager.DeleteData("some-key");

// Delete data asynchronously
await GameManager.DeleteDataAsync("some-key");

// Read Editor data
if (GameManager.TryReadEditorData<MyData>("some-key", out var data))
{
}

// Save Editor data
GameManager.SetEditorData("some-key", data);

// Delete Editor data
GameManager.DeleteEditorData("some-key");

// Deserialize a value from a string
if (GameManager.TryDeserializeValue<MyValue>("{}", out var value))
{
}

// Deserialize a value from a Stream
Stream stream = ...;
if (GameManager.TryDeserializeStream<MyValue>(stream, out var value))
{
}

// Serialize a value to a string
if (GameManager.TrySerializeValue(value, out var serializedValue))
{
}

// Serialize a Stream to a string
Stream stream = ...;
if (GameManager.TrySerializeStream(stream, out var serializedValue))
{
}
```
