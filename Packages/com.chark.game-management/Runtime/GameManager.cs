...
namespace CHARK.GameManagement
{
    ...
    public abstract partial class GameManager : MonoBehaviour
    {
        ...
        private bool hasReportedUninitializedManager;
        ...
        /// <summary>
        /// Shout once when this instance somehow ended up ticking without having been initialized,
        /// instead of throwing a NullReferenceException every single frame.
        /// </summary>
        /// <remarks>
        /// This happens when a script is recompiled while Play mode is running and "Enter Play Mode
        /// Options" has Reload Domain turned off: the running manager loses its managed state, so
        /// <see cref="entityManager"/> becomes null. One clear warning is worth more than a few
        /// thousand exceptions.
        /// </remarks>
        private void ReportUninitializedManager()
        {
            if (hasReportedUninitializedManager)
            {
                return;
            }

            hasReportedUninitializedManager = true;

            Debug.LogWarning(
                $"{nameof(GameManager)} is ticking without being initialized ("
                + $"{nameof(entityManager)} is null), so it is skipped. Restart Play mode, and turn"
                + " off 'Enter Play Mode Options' in Project Settings > Editor if this keeps happening.",
                this
            );
        }
