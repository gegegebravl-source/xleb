...
namespace CHARK.ScriptableScenes
{
    ...
    public sealed class ScriptableSceneCollection : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        public IEnumerator ShowTransitionRoutine(bool isFirstLoad = false)
        {
            if (transition)
            {
                var args = new CollectionTransitionEventArgs(isFirstLoad);
                collectionEvents.RaiseShowTransitionEntered(args);
                yield return transition.ShowRoutine(isFirstLoad: isFirstLoad);
                collectionEvents.RaiseShowTransitionExited(args);
            }
        }
        ...
        /// </returns>
        public IEnumerator HideTransitionRoutine(bool isFirstLoad = false)
        {
            if (transition)
            {
                var args = new CollectionTransitionEventArgs(isFirstLoad);
                collectionEvents.RaiseHideTransitionEntered(args);
                yield return transition.HideRoutine();
                collectionEvents.RaiseHideTransitionExited(args);
            }
        }
